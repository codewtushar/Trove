...
final categoryCountProvider = FutureProvider.family((ref, String category) async {
  final user = ref.watch(authProvider).value;
...
final warrentyProvider = StreamProvider<int>((ref){
  ...
  return FirebaseFirestore.instance.collection('users').doc(user!.uid).collection('assets').snapshots().map((snap){
    int count = 0;
...
final categoryAssetsProvider  = StreamProvider.family((ref, categoryName){
  ...
  if(user == null) return const Stream.empty();
   ...
   return FirebaseFirestore.instance.collection('users').doc(user.uid).collection('assets')
       .where('category', isEqualTo: categoryName).snapshots();
});
